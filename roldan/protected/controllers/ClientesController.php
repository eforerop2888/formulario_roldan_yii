...
class ClientesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Clientes'],$_POST['Direcciones']))
		{
			$model->attributes=$_POST['Clientes'];
			$model_direcciones->attributes=$_POST['Direcciones'];
			
			$model_direcciones->id_clientes=4;
			$valid = $model->validate();
			$valid = $model_direcciones->validate() && $valid;
			
			if ($valid){
				if($model->save()){
					$model_direcciones->id_clientes = $model->id_clientes;
		...
		$this->render('create',array(
			'model'=>$model,
			'model_direcciones'=>$model_direcciones,
		));
	}
